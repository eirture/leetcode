...
简单思路：
* 针对两个链表，m * n 依次比较
* hash 表记录一条链表元素，遍历另一条链表并查看 hash 表。

### 2. 双指针

![](https://pic.eirture.cn/pics/IMG_5703.jpg)

假设两张链表相交，`a`, `b` 尾部长度一定相同（相交以后，就是同一条子链表）。两个指针分别同时遍历 `a+b` 和 `b+a` 的链表，一定会相交。或者同时走到尾部 None。
