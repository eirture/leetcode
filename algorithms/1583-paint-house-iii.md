# [1583. 粉刷房子 III](https://leetcode-cn.com/problems/paint-house-iii/) - hard
...
1. 已经有颜色 j，不增加花费，如果 house[i - 1] == j 则 target 不用变，否则 target - 1
2. 没有颜色，计算涂成 0-n 颜色花费最少情况.
...
class Solution:
    def minCost(self, houses: List[int], cost: List[List[int]], m: int, n: int, target: int) -> int:
        ...
        def dfs(i, t, c):
            ...
            if houses[i] != 0:
                return dfs(i + 1, t if houses[i] == c else t - 1, houses[i])
            else:
                return min(dfs(i + 1, t if j + 1 == c else t - 1, j + 1) + cost[i][j] for j in range(n))
