# [1605. 制作 m 束花所需的最少天数](https://leetcode-cn.com/problems/minimum-number-of-days-to-make-m-bouquets/) - medium
...
首先判断如果 len(bloomDay) < m * k 直接返回 `-1`。否则最终结果在 [min(bloomDay), max(bloomDay)] 之间。通过二分法找到这个结果。

创建一个函数 `isok(d: int) -> bool` 判断第 d 天是否满足。通过二分找到最小满足的天数。
...
class Solution:
    def minDays(self, bloomDay: List[int], m: int, k: int) -> int:
        ...
        mind, maxd = min(bloomDay), max(bloomDay)
